...
In `stop`mode, the speed value will be compared to 0.2 to either determine if the Rover's still moving. If True, brakes will be further applied (0.5 gain in line 96) in order for the Rover to stop. Once stopped, image processing will determine if navigation angles' array length is lower than the `Rover.stop_forward` threshold (line 101), or if on the contrary it is higher than the `Rover.go_forward` element (line 108). The former statement allows to figure out if the Rover has reached a dead end (see image below), while the latter provides any feedback whether it is possible to continue its journey (and this applies also to the case where `Rover.rock_in_the_middle` was previously set to True). In comparison with the sample code, line 101 was modified so the Rover could set reverse and return to the navigable path (this with the intention to take the Rover out of an already followed path).
...
As a conclusion, it was a challenge to take to the Search and Sample project this far with flow-control functions, hence determining the actions the Rover should follow step-by-step. Nonetheless, it would have been good to have some sort of an idea to properly implement the returning to the point of departure step plus that of the already explored areas, but I guess it's part of the learning process.
