...
Finally, the combination shown in line 13 of `model_training.html` (including the 5-by-5 encoders-decoders model) was the one that after many trials reached an improved global IOU of 35%, although well below the required passing threshold. Once more, the batch size was modified to 30 (plus the corresponding steps-per-epoch) in other to improve the training time, but the model couldn't shape neither training nor validation data after the 10th epoch; it was `underfitted` as the image below shows. From this point onwards, the batch-size was increased, and training started to be performed in AWS servers in order to speed-up the process while testing another combination of parameters.
...
2. Skip connections were implemented in line 9 of `model_training.html`. However, modify the number of connections would be nice to experiment with. Given the size of the proposed network and the time it takes for 15 epochs to run locally, a comparison between the whole FCN results and that if some connections were skipped would be useful to understand how much data is really necessary to reach the 0.40 global precision, and how much it can be improved from this point. 
3. A 1x1 convolution is implemented in the model. Would be good to understand until what point the insertion of such step is benefitial for my model in terms of runtime. 
4. The model could be extrapolated to detect objects different than humans, but would it be possible? At a first glance, it would depend on the amount of data to train with and the amount of pixels to classify from. As a matter of fact, detected humans were either a 'hero' or clutter. In such case, a car/dog/cat could be detected if the detecting format is the same (only one hero among clutter), but if we're looking forward to detect a specific car(s) (automobile, SUV, etc) or dog(s) (Schanuzer, Yorkshire Terrier, etc) the algorithm should be capable of separating the 'hero' from clutter by either increasing the training data or deepen the network. It would all depend on whatever the results are after experimentation.
